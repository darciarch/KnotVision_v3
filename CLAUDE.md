...
.venv/bin/python -m img2texcelle.split data/design.jpg --parts 2 --axis lr --symmetric   # cut on the real mirror axis (crops a few px)
```

`img2texcelle.split` writes lossless PNGs to `data/cropped_images/<name>/<name>_<part>.png`, one folder per source image; a later run on the same image adds to that folder with `_2`, `_3`, ... (nothing is ever overwritten) and the source is left where it is. `--keep` is a comma-separated list of part names (default all). An odd side puts its middle pixel row/column into both parts (a warning is printed), so the parts always have the same size. The cut is at the geometric centre; `--symmetric` finds the real mirror axis of each cut axis (`symmetry.find_axis_shift`: the `detect_symmetry` search without its threshold, then `refine_axis_shift`) and crops the image with `crop_to_axis` so the axis is the centre, which makes the parts bit-identical mirrors (measured: Gemini renders are off by 0–2.5 px left/right); an axis with error ratio > 0.35 is not trusted and stays at the centre with a warning. `--shift DX,DY` sets the shift by hand (`detect_symmetry` sign convention).
...
img2texcelle/
  ...
  symmetry.py   detect_symmetry, refine_axis_shift, crop_to_axis, find_and_centre, mirror_average, mirror_copy
...
1. **Orientation, symmetry, aspect** (`grid.rotate_to_carpet`, `symmetry.find_and_centre`, `grid.fit_to_carpet`, `grid.source_scale`). If the image and the carpet have different orientations, rotate the image 90°. `detect_symmetry` compares the greyscale (1/4 size) with its mirror (best shift within ±2% of the size): an axis is symmetric when the error is below 0.35× the error of an 8 px shift (measured 2026-09-16: Gemini renders 0.07–0.27 left/right and 0.76–0.94 top/bottom, the real design 0.00–0.02 both ways). An off-centre axis is refined at full resolution and the image is cropped so the axis is the centre of the knot grid. Symmetric axes then act in two places: the knot-grid coverage maps are averaged with their mirror (`mirror_average`), and after cleanup the left/top half is copied onto the right/bottom (`mirror_copy`), which makes opposite motifs bit-identical (without it a Gemini output matched its own mirror on only 93.8% of the knots). Caveat: a design that is *almost* symmetric about an axis gets that axis detected too, and a motif present on one side only is then averaged away; force `--symmetry lr` in that case. The image ratio must match the carpet ratio in cm or the knot-grid ratio within 2%, otherwise `--fit crop` / `--fit stretch` or an error. `compute_pixels` gives the knot grid plus the points per meter written into the header. `scale = sqrt(sx*sy)` = source px per knot must be > 1, otherwise an error.
