...
- **Source discipline**: the only input is a plain image file (jpg in practice): square pixels, aspect ratio within 10% of the carpet's cm ratio, flat-colored (no shadows, gradients, bevels), more pixels than knots in both directions. No other input exists (no Texcelle files, no knot-scaled images). Below 10% the image is resized onto the knot grid and the distortion is printed; above 10% it is an error. Fewer pixels than knots in either direction is an error. A shaded source turns bevels into same-hue slivers in the output; the only remedy is a flat source.
...
Tuning flags: `--merge` (delta E for merging auto colors, default 6), `--min-area` (knots; default 20 mm² of knots: 4 at 397×500), `--symmetry` (`auto` / `none` / `lr` / `tb` / `both`, default `auto`: the left/top half of a mirror-symmetric design is copied onto the other, so opposite motifs are identical; halves that match everywhere are also decided from both together; forced modes use the measured axis position), `--format` (`tiff` / `bmp`, default `tiff`), `--debug-dir` (writes `regions.png`, the label map before cleanup). `--width`, `--height`, `--reed` and `--density` are required.

A run on a 3392×5056 source at 397×500 takes ~4 min; a 1700×2500 source ~60 s.
...
img2texcelle/
  ...
  grid.py       compute_pixels (knot grid + header ppm), knot size, rotate, check_aspect (10%), source_scale
...
tests/          test_smoke.py (synthetic 2:3 design end to end, aspect and scale rules), test_workspace.py, test_split.py, test_symmetry.py
```
...
- **Off-centre axis** (references made 2026-09-17): `data/fomggggggbro/fomggggggbro.jpg` (3392×5056, medallion 128 px above the centre) with `--width 200 --height 300 --reed 397 --density 500 --palette` from its `_6_palette.txt` `--format bmp` must print the left/right axis at the centre (contrast 0.12, average + copy), the top/bottom axis at row 2399.5 (contrast 0.57, copy only), the crop to 3392×4800 and `6.0% distortion`, and give an output that is pixel-identical to `fomggggggbro_3.bmp`: mirror-symmetric both ways with no ghost motifs at the crowns. The same command with `--symmetry lr` must print `0.6% distortion` and be pixel-identical to `fomggggggbro_6.bmp` (left/right symmetric only). `python -m img2texcelle.split ... --parts 2 --axis tb --symmetric` on the same image gives a 3392×2400 top and a 3392×2656 bottom part, and the top part mirrored at its bottom edge lines up with the medallion. The three quarter images in `data/cropped_images/fomggggggbro/` must be rejected on both axes (contrast ≥ 0.79).
